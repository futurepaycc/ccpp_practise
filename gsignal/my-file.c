...
enum PropertyDList { PROPERTY_FILE_0, PROPERTY_FILE_NAME };

static void my_file_dispose(GObject *gobject) {
...
static void my_file_finalize(GObject *gobject) {
    MyFile *self = MY_FILE(gobject);
    MyFilePrivate *priv = MY_FILE_GET_PRIVATE(self);
...
static void my_file_class_init(MyFileClass *klass) {
    ...
    GParamSpec *pspec;
    pspec = g_param_spec_string("name", "Name", "File name", NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT);
